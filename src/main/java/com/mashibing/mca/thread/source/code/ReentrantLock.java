package com.mashibing.mca.thread.source.code;
...
public class ReentrantLock implements Lock, java.io.Serializable{
    ...
    static final class NonfairSync extends Sync {
        ...
        final boolean initialTryLock() {
            Thread current = Thread.currentThread();
            //和公平锁相比，这里会直接先进行一次CAS，成功就返回true了。
            //通过CAS操作将状态值state由0改为1如果修改成功，将当前线程设置为独占状态，
            // 然后返回true。因为lock()方法是（！initialTryLock()）所以就直接表示获取锁成功
            if (compareAndSetState(0, 1)) { // first attempt is unguarded
               //表示将当前线程设置为独占状态。
                setExclusiveOwnerThread(current);
                return true;
                //判断当前的线程是不是已经持有了锁，如果是同一个就表示重入锁，
                // 然后将转状态值state加1，并返回true表示获取锁成功。
            } else if (getExclusiveOwnerThread() == current) {
                int c = getState() + 1;
                if (c < 0) // overflow
                    throw new Error("Maximum lock count exceeded");
   ...
   //默认使用非公平锁，因为考虑获取更好的性能。
     ...
     */
    //通过boolean类型来决定是否采用公平锁（传入true就是公平锁）
